...
H<=8なのでどのA個を選ぶかbit全探索する。計算量はO(2^H * WlogW)になる。
```C++
...
```
`(i & (1<<j))`<-ビット全探索のテクニック
## 70. 二次元空間上のマンハッタン距離の総和を最小にする
> マンハッタン距離：空間を格子状とみなした時の道なりの距離

重心を求めることは想像できるが……<-中央値だった
### 平均値は二乗誤差最小， 中央値は絶対誤差最小
##　72. 二次元空間上で同じマスを通らずにkマスの閉路を作る。kの最大値は？
全探索という感覚はわかったが実装がわからなかった。
### 再帰関数を使ったバックトラック
実装が大変。既に通った道かどうかを判断するための配列を用意する。
## 85. K=abcとなるa,b,c(a<=b<=c)の組み合わせ数は？✌️
約数列挙のテクニックは素因数分解と同様。
---
